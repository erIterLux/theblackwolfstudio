...
export default function InstructorReportsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setMessage('');
    try {
      const result = await getStudioReportSummary(payload);
      setData(result);
      setRepairPreview(null);
    } catch (nextError) {
      console.error('Studio reports could not be loaded:', nextError);
      setError(nextError?.message || 'Studio reports could not be loaded.');
    } finally {
      setLoading(false);
    }
  }, [isInstructor, payload]);

  useEffect(() => {
    if (roleLoading || !isInstructor) return;
    queueMicrotask(load);
  }, [roleLoading, isInstructor, load]);
        ...
        <div className="report-range-panel">
          ...
          {preset === 'custom' && (
            <div className="report-custom-range">
              ...
              <button type="button" className="button button--primary" onClick={load} disabled={!custom.startDate || !custom.endDate}>Apply</button>
            </div>
          )}
          ...
          <div className="report-section-stack">
            ...
            <div className="report-two-column">
              <article className="report-panel">
                ...
                <button type="button" className="text-link report-link-button" onClick={() => selectTab('engagement')}>Review member attention list</button>
              </article>
              <article className="report-panel">
                ...
                <button type="button" className="text-link report-link-button" onClick={() => selectTab('systemHealth')}>Open system health</button>
              </article>
            </div>
