...
export default function InstructorProgressionAdmin() {
  ...
  const uploadForMember = async (categoryKey, video) => {
    ...
    try {
      ...
      if (result?.previousStoragePath && result.previousStoragePath !== video.storagePath) {
        await deleteProgressionVideo(result.previousStoragePath).catch((error) => {
          console.warn('Old instructor-uploaded video was not removed:', error);
        });
      }
      await loadDetail(detail.review.id);
      setMessage('Evidence video updated. Review the new video before validating the category.');
    } catch (error) {
      console.error(error);
    ...
    <section className="progression-admin-page">
      <div className="container progression-admin-shell">
        ...
        <header className="progression-admin-header">
          <div><p className="eyebrow">Instructor workspace</p><h1>Progression review</h1><p>Validate one category at a time. A level cannot be approved until all seven categories are validated.</p></div>
                      ...
                      <article className="progression-admin-category" key={category.key}>
                        ...
                        <label className="progression-notes">
                          ...
                          <textarea
                            ...
                            placeholder="Record what was demonstrated and the next specific correction, if any."
