...
function sanitizeEvent(data, instructorUid) {
  ...
  const pricePerParticipantCents = cents(data?.pricePerParticipantCents);
  if (!pricePerParticipantCents) {
    throw new HttpsError('invalid-argument', 'Enter an event price per participant.');
  }
