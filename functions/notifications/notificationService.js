...
async function handleListMyNotifications(request) {
  const uid = requireAuthenticated(request);
  const limit = Math.min(150, Math.max(20, Number(request.data?.limit || 100)));
  const includeRead = request.data?.includeRead !== false;
  const snapshot = await db.collection('users').doc(uid).collection('notifications')
    .orderBy('createdAt', 'desc')
    .limit(limit)
    .get();
  const all = snapshot.docs.map((item) => serialize({ id: item.id, ...item.data() }));
  const notifications = includeRead ? all : all.filter((item) => item.status === 'unread');

  let unreadCount = all.filter((item) => item.status === 'unread').length;
  try {
    const countSnapshot = await db.collection('users').doc(uid).collection('notifications')
      .where('status', '==', 'unread')
      .count()
      .get();
    unreadCount = Number(countSnapshot.data().count || 0);
  } catch (error) {
    logger.warn('Notification unread aggregate failed; using loaded result count.', {
      uid,
      error: error?.message,
    });
  }
...
async function handleMarkNotificationRead(request) {
  ...
  return { success: true, status: read ? 'read' : 'unread' };
}
