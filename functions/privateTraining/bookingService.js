...
function makeSlotsForDate({ availability, override, day, dateKey, durationMinutes, excludeBookingId = '' }) {
    ...
    const slots = [];
    const diagnostics = {
        blockedDate: override?.mode === 'blocked',
        windowCount: windows.length,
        shortWindowCount: 0,
        candidateCount: 0,
        outsideBookingWindowCount: 0,
        conflictCount: 0,
    };
    ...
    for (const window of windows) {
        ...
        if (windowEndMinutes - windowStartMinutes < durationMinutes) {
            diagnostics.shortWindowCount += 1;
            continue;
        }
        for (
            ...
            if (startsAt.getTime() < earliest || startsAt.getTime() > maxDate) {
                diagnostics.outsideBookingWindowCount += 1;
                continue;
            }
            if (intervalOverlaps(intervals, startsAt, endsAt, bufferMinutes, excludeBookingId)) {
...
async function handleListPrivateTrainingAvailability(request) {
    ...
    const allAvailabilitySnapshot = await db.collection('instructorAvailability')
        .limit(50)
        .get();
    const requestedInstructorUid = clean(request.data?.instructorUid, 160);
    const allInstructors = allAvailabilitySnapshot.docs
        .filter((item) => !requestedInstructorUid || item.id === requestedInstructorUid)
        .map((item) => ({ id: item.id, ...item.data() }));
    const activeInstructors = allInstructors.filter((item) => item.active === true);
    const instructors = activeInstructors.filter((item) => (
        !item.supportedDurations?.length || item.supportedDurations.includes(durationMinutes)
    ));

    const basePurchase = serialize({
        id: purchase.id,
    ...
    if (!instructors.length) {
        return {
            purchase: basePurchase,
            instructors: activeInstructors.map((item) => serialize({
                uid: item.id,
            ...
            slots: [],
            availabilityState: {
                code: 'duration_not_enabled',
                durationMinutes,
                supportedDurations: [...new Set(
                    activeInstructors.flatMap((item) => item.supportedDurations || []),
                )].sort((left, right) => left - right),
            },
        };
    }
    ...
    for (const instructor of instructors) {
        ...
        for (const dateKey of dates) {
            ...
            totals.blockedDateCount += result.diagnostics.blockedDate ? 1 : 0;
            totals.openDateCount += result.diagnostics.windowCount ? 1 : 0;
            totals.windowCount += result.diagnostics.windowCount;
            totals.shortWindowCount += result.diagnostics.shortWindowCount;
            totals.candidateCount += result.diagnostics.candidateCount;
            totals.outsideBookingWindowCount += result.diagnostics.outsideBookingWindowCount;
            totals.conflictCount += result.diagnostics.conflictCount;
    ...
    if (!slots.length) {
        if (!totals.openDateCount) stateCode = 'no_hours_in_range';
        else if (!totals.candidateCount && totals.shortWindowCount) stateCode = 'hours_shorter_than_session';
        else if (
            totals.candidateCount > 0
            && totals.outsideBookingWindowCount === totals.candidateCount
        ) stateCode = 'outside_booking_window';
        else if (
            totals.candidateCount > 0
            && totals.conflictCount === totals.candidateCount
        ) stateCode = 'fully_booked';
        else stateCode = 'no_open_slots';
    }
    ...
    return {
        ...
        instructors: instructors.map((item) => serialize({
            ...
            supportedDurations: item.supportedDurations || [],
            minNoticeHours: item.minNoticeHours ?? 12,
            maxAdvanceDays: item.maxAdvanceDays ?? 60,
        })),
        slots,
        availabilityState: {
            code: stateCode,
