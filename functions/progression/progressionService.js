...
async function readProgression(memberUid) {
  ...
  for (const levelDoc of levelSnaps.docs) {
    ...
    const categories = Object.fromEntries(
      categorySnap.docs.map((docSnap) => [docSnap.id, serialize({ id: docSnap.id, ...docSnap.data() })]),
    );
    levels.push(serialize({ id: levelDoc.id, ...levelDoc.data(), categories }));
  }
...
async function handleSaveProgressionCategory(request) {
  ...
  let previousStoragePath = categoryData.video?.storagePath || null;
  if (video) {
    const expectedPrefix = `progression-videos/${memberUid}/${levelKey}/${categoryKey}/`;
    const storagePath = clean(video.storagePath, 900);
    if (!storagePath.startsWith(expectedPrefix)) {
      throw new HttpsError('invalid-argument', 'The uploaded video path is invalid.');
    }
    updates.video = {
      storagePath,
    ...
    updates.status = categoryData.status === 'validated' ? 'validated' : 'in_practice';
  ...
  if (['active', 'needs_work'].includes(levelData.status)) {
  ...
  return { success: true, previousStoragePath };
}
...
async function handleGetProgressionReview(request) {
  ...
  return serialize({
    ...
    categories: Object.fromEntries(categoriesSnap.docs.map((docSnap) => [
      docSnap.id,
